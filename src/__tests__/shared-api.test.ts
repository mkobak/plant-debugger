...
import { checkRateLimit, addRateLimitDelay, processFormData, convertImagesToBase64, validateImages, getClientId } from '@/lib/api/shared';
...
describe('shared api helpers', () => {
    ...
    it('processFormData extracts images and fields', async () => {
        ...
        const f1 = new File([new Uint8Array([1, 2, 3])], 'a.jpg', { type: 'image/jpeg' });
    ...
    it('validateImages throws on empty', () => {
        expect(() => validateImages([] as unknown as File[])).toThrow('No images provided');
    });

    it('getClientId prefers x-pb-client-id then x-forwarded-for then x-real-ip', () => {
        const makeReq = (headers: Record<string, string>) => ({ headers: { get: (k: string) => headers[k] || null } }) as any;
