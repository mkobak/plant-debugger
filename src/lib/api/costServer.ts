...
interface Totals {
  ...
  inputCost: number;
  outputCost: number;
  byModel: Record<ModelKey, { calls: number; input: number; output: number; cost: number }>;
}
...
function ensure(id: string): Totals {
  ...
  if (t) return t;
  const v: Totals = { 
    prompt: 0, output: 0, inputCost: 0, outputCost: 0,
    byModel: {
...
export function recordUsageForRequest(req: NextRequest, modelKey: ModelKey, usage: UsageMetadata | undefined) {
  ...
  m.output += ct;
  m.cost += incIn + incOut;
  console.log(`(CostServer) ${modelKey}@${id}: input ${pt}, output ${ct}, cost ~$${(incIn + incOut).toFixed(4)}`);
}

export function printAndResetForRequest(req: NextRequest, context = 'Diagnosis complete') {
  const id = getClientId(req);
  ...
  if (unknown) {
    ...
    (Object.keys(unknown.byModel) as Array<keyof typeof unknown.byModel>).forEach((k) => {
  ...
  console.log(callsLine);
  console.log(`- modelHigh: ${totals.byModel.modelHigh.calls} calls (~$${totals.byModel.modelHigh.cost.toFixed(4)})`);
  console.log(`- modelMedium: ${totals.byModel.modelMedium.calls} calls (~$${totals.byModel.modelMedium.cost.toFixed(4)})`);
  console.log(`- modelLow: ${totals.byModel.modelLow.calls} calls (~$${totals.byModel.modelLow.cost.toFixed(4)})`);
  console.log(`Input tokens: ${totals.prompt.toLocaleString()} (~$${totals.inputCost.toFixed(4)})`);
  console.log(`Output tokens: ${totals.output.toLocaleString()} (~$${totals.outputCost.toFixed(4)})`);
