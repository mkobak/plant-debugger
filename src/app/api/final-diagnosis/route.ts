...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!checkRateLimit(clientId)) {
      console.warn(`[FINAL-DIAGNOSIS:${requestId}] Rate limit exceeded for client: ${clientId}`);
    ...
    if (signal?.aborted) {
      console.warn(`[FINAL-DIAGNOSIS:${requestId}] Aborted before reading form data`);
  ...
  console.log(`[FINAL-DIAGNOSIS:${requestId}] Client: ${clientId} | images: ${images.length} (~${Math.round(totalImageBytes/1024)} KB) | Q&A len: ${questionsAndAnswers?.length || 0} | ranked len: ${rankedDiagnoses?.length || 0}`);

    // Additional protection: if there are multiple rapid requests, add a delay
    await addRateLimitDelay(clientId);

    if (signal?.aborted) {
      console.warn(`[FINAL-DIAGNOSIS:${requestId}] Aborted before converting images`);
      return NextResponse.json({ error: 'Request canceled' }, { status: 499 });
    }

    // Convert images to base64 for Gemini
    const imageParts = await convertImagesToBase64(images);
  console.log(`[FINAL-DIAGNOSIS:${requestId}] Converted ${imageParts.length} images to base64`);
  ...
  console.log(`[FINAL-DIAGNOSIS:${requestId}] Sending to AI | prompt len: ${MAIN_DIAGNOSIS_PROMPT.length} | images: ${imageParts.length} | tools: ${tools.length}`);
  ...
  const result = await new Promise<typeof genPromise extends Promise<infer R> ? R : never>((resolve, reject) => {
      ...
      if ((err as Error)?.message === 'aborted') {
        console.warn(`[FINAL-DIAGNOSIS:${requestId}] Aborted during model call`);
  ...
  printResponse(`[FINAL-DIAGNOSIS:${requestId}]`, response);
    ...
    console.log(`[FINAL-DIAGNOSIS:${requestId}] AI response received | candidates: ${response.candidates?.length || 0}`);
  ...
  const partsSummary = (candidate as any)?.content?.parts?.map((p: any) => Object.keys(p)).flat() || [];
    console.log(`[FINAL-DIAGNOSIS:${requestId}] Candidate summary | finish: ${finishReason} | parts: ${partsSummary.join(',')}`);
    
    if (!candidate.content || !candidate.content.parts) {
      const fallbackText = response.text();
      console.error(`[FINAL-DIAGNOSIS:${requestId}] No content parts in response`);
      console.error(`[FINAL-DIAGNOSIS:${requestId}] Raw text (excerpt):`, typeof fallbackText === 'string' ? fallbackText.slice(0, 4000) : String(fallbackText));
    ...
    if (functionCallPart && functionCallPart.functionCall) {
      console.log(`[FINAL-DIAGNOSIS:${requestId}] Function call found: ${functionCallPart.functionCall.name}`);
      
      if (functionCallPart.functionCall.name === 'plant_diagnosis') {
        diagnosisData = functionCallPart.functionCall.args;
  console.log(`[FINAL-DIAGNOSIS:${requestId}] Extracted args keys: ${Object.keys(diagnosisData || {}).join(', ')}`);
      } else {
      ...
      console.log(`[FINAL-DIAGNOSIS:${requestId}] No function call found, trying to parse text as JSON`);
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          console.log(`[FINAL-DIAGNOSIS:${requestId}] Parsed JSON from text with keys: ${Object.keys(diagnosisData || {}).join(', ')}`);
        } else {
          throw new Error('No JSON found in response and no function call');
        }
      } catch (parseError) {
        console.error(`[FINAL-DIAGNOSIS:${requestId}] Failed to parse JSON from text:`, parseError);
        if (typeof responseText === 'string') {
          console.error(`[FINAL-DIAGNOSIS:${requestId}] Raw text (excerpt):`, responseText.slice(0, 4000));
        }
        // Provide compact candidate parts overview for debugging
  const partsOverview = response.candidates?.map((c: any, i: number) => ({ i, partKinds: (c.content?.parts||[]).map((p: any) => Object.keys(p)).flat() })) || [];
  console.error(`[FINAL-DIAGNOSIS:${requestId}] Candidates overview:`, safeStringify(partsOverview));
        throw new Error('Invalid diagnosis response format');
      }
    }

    console.log(`[FINAL-DIAGNOSIS:${requestId}] Parsed diagnosis keys: ${Object.keys(diagnosisData || {}).join(', ')}`);
