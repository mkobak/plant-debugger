...
import { processFormData, convertImagesToBase64, validateImages, getClientId } from '@/lib/api/shared';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (signal?.aborted) {
  console.warn(`[IDENTIFY-PLANT:${requestId}] Aborted before reading form data`);
      return NextResponse.json({ error: 'Request canceled' }, { status: 499 });
    }

    const clientId = getClientId(request);
    if (inFlightByClient.has(clientId)) {
  console.warn(`[IDENTIFY-PLANT:${requestId}] Concurrent request blocked for client ${clientId}`);
      return NextResponse.json({ error: 'Identification already in progress' }, { status: 429 });
    }
  ...
  console.log(`[IDENTIFY-PLANT:${requestId}] images: ${images.length} (~${Math.round(totalImageBytes/1024)} KB)`);

    if (signal?.aborted) {
  console.warn(`[IDENTIFY-PLANT:${requestId}] Aborted before converting images`);
      return NextResponse.json({ error: 'Request canceled' }, { status: 499 });
    }

    // Convert images to base64 for Gemini
    const imageParts = await convertImagesToBase64(images);
  console.log(`[IDENTIFY-PLANT:${requestId}] Converted ${imageParts.length} images to base64`);

  // Print prompt exactly once (gated)
  printPrompt(`[IDENTIFY-PLANT:${requestId}]`, PLANT_IDENTIFICATION_PROMPT);

    // Log what we're sending to AI
  console.log(`[IDENTIFY-PLANT:${requestId}] Sending to AI | images: ${imageParts.length}`);
  ...
  console.log(`[IDENTIFY-PLANT:${requestId}] Extracted plant name: ${plantName}`);
  ...
  console.warn(`[IDENTIFY-PLANT:${requestId}] Falling back to empty identification due to model error`);
